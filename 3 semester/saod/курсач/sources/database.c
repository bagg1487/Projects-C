...
void print_aligned(const char* s, int width) {
    int real = utf8_display_width(s);
    printf("%s", s);
    for (int i = real; i < width; i++) {
        putchar(' ');
    }
}
...
int load_database(const char* filename, Record*** records) {
    ...
    *records = (Record**)malloc(count * sizeof(Record*));
    if (!*records) {
        fclose(file);
        return 0;
    }
    ...
    for (int i = 0; i < count; i++) {
        if (fread(buffer, 1, record_size, file) != record_size) break;
...
void print_records_table(Record** records, int start, int end) {
    ...
    for (int i = start; i < end; i++) {
        ...
        print_aligned(records[i]->fio, 30);
        printf(" │ ");

        print_aligned(records[i]->street, 16);
        printf(" │ %-4d │ %-7d │ ", records[i]->house, records[i]->apartment);

        print_aligned(records[i]->date, 10);
        printf(" │\n");
    }
